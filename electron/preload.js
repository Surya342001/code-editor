...
contextBridge.exposeInMainWorld('api', {
  ...
  gitSyncMain:      (dir)             => ipcRenderer.invoke('git:syncMain',     dir),
  gitPublishBranch: (dir)             => ipcRenderer.invoke('git:publishBranch', dir),
  gitPullRebase:    (dir)             => ipcRenderer.invoke('git:pullRebase',    dir),
  gitRebaseMain:    (dir)             => ipcRenderer.invoke('git:rebaseMain',    dir),
  gitAbortRebase:   (dir)             => ipcRenderer.invoke('git:abortRebase',   dir),
  gitPrUrl:         (dir, branch, remote) => ipcRenderer.invoke('git:prUrl',     dir, branch, remote),
  gitBranches:      (dir)             => ipcRenderer.invoke('git:branches',     dir),
  gitCreateBranch:  (dir, name)       => ipcRenderer.invoke('git:createBranch', dir, name),
  gitSwitchBranch:  (dir, name)       => ipcRenderer.invoke('git:switchBranch', dir, name),
  gitDeleteBranch:  (dir, name, force)=> ipcRenderer.invoke('git:deleteBranch', dir, name, force),
  gitStash:         (dir, msg)        => ipcRenderer.invoke('git:stash',        dir, msg),
  gitStashPop:      (dir)             => ipcRenderer.invoke('git:stashPop',     dir),
  gitStashDrop:     (dir, idx)        => ipcRenderer.invoke('git:stashDrop',    dir, idx),
  gitStashList:     (dir)             => ipcRenderer.invoke('git:stashList',    dir),
  gitRemotes:       (dir)             => ipcRenderer.invoke('git:remotes',      dir),
  gitAddRemote:     (dir, name, url)  => ipcRenderer.invoke('git:addRemote',    dir, name, url),
});
